  ...
  var reload = function() {
    ...
    $.get(
      'https://api.openweathermap.org/data/2.5/weather?lat=' +
        ...
        '&lon=' +
        lon +
        '&units=imperial&cnt=6&APPID=1a0075ceb23c8596e0371403a7831774',
      function(response) {
    ...
    $.get(
      ...
      function(response) {
        ...
        $('#hi-lo').text(
          'Hi: ' +
            Math.round(celCon(response.list[0].temp.max)) +
            ' | Lo: ' +
            Math.round(celCon(response.list[0].temp.min))
        );
        for (var i = 1; i <= 5; i++) {
          //set min/max forecasted temps for each day
          $('#mini-temp' + i).text(
            Math.round(celCon(response.list[i].temp.max)) + '/' + Math.round(celCon(response.list[i].temp.min))
          );
